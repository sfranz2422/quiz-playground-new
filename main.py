...
SELECT *
FROM (
    SELECT DISTINCT ON (questionSetTitle, quizId)
        quizId,
        questionSetTitle,
    ...
    WHERE teacherId = %s
    ORDER BY questionSetTitle, quizId, timestamp DESC
) AS latest_quizzes
...
def videoQuestions(teacherid, set, number):
    ...
    if request.method == "POST" and 'answer' in request.form:
        ...
        if request.form['answer'] == request.form['correctanswer']:
            ...
            requests.get(f"{request.host_url}recordAnswer/{questionid}/1/0/{teacherid}/{set}/{questionsettitle}/{questiontext}")
            # flash("Correct!")
            # return redirect(url_for('coinDash'))
        
        else:
            print("Sorry wrong choice")
            requests.get(f"{request.host_url}recordAnswer/{questionid}/0/1/{teacherid}/{set}/{questionsettitle}/{questiontext}")
